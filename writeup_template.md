...
Initially, a fourth grid of 256px was created, but that one proved to work really bad on video streams, producing only false positives most of the time, so it was finally removed. A reason for this might be that the non-car examples available to train our classifier do not include images similar to the ones that would be generated by such a big window.
